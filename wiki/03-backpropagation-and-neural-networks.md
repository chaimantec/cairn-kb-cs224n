...
where $x$ is the input, $W$ the weight matrix, $b$ the bias, and the activation $f$ is
applied element-wise, one scalar at a time
(≈7:01). The running example for the whole lecture is the named entity window
classifier from lecture 2 (slide 5): a five-word window, each word's $d$-dimensional
embedding concatenated into $x \in \mathbb{R}^{5d}$, one hidden layer, and a dot product
$u^{\top} h$ producing a single score $s$ that feeds a logistic function. Manning computes
gradients of the **score** rather than the loss throughout, because the loss's derivative is
an Assignment 2 question he does not want to give away (slide 27, ≈31:45).

## Non-linearities: why, and which

The very first artificial neuron, McCulloch and Pitts' 1943 threshold unit, fired when
$Wx$ exceeded a threshold $\theta$ (slide 6). That function is flat on both sides, so it has no
...
$W_1 W_2 x = W x$ — so a deep network of pure matrix multiplies has exactly the
...
- One input, one output: the gradient is the ordinary derivative. For $f(x) = x^3$ it
  is $3x^2$, and the derivative tells you how much the output moves when you nudge the
...
- $n$ inputs, $m$ outputs: the gradient is the **Jacobian**, an $m \times n$ matrix with
  $(\partial f / \partial x)_{ij} = \partial f_i / \partial x_j$. This is the shape a neural
  network layer has, which is why Jacobians are the working object here (≈24:43).
...
| Expression | Jacobian |
| --- | --- |
| $h = f(z)$, element-wise | $\operatorname{diag}(f'(z))$ |
| $\dfrac{\partial}{\partial x}(Wx + b)$ | $W$ |
| $\dfrac{\partial}{\partial b}(Wx + b)$ | $I$ |
| $\dfrac{\partial}{\partial u}(u^{\top} h)$ | $h^{\top}$ |
...
Slides 26–37 compute $\partial s / \partial b$ in four steps, and the steps are the method:

1. **Break the equations into simple pieces**, defining each variable and tracking its
   dimensionality: $s = u^{\top} h$, $h = f(z)$, $z = Wx + b$.
2. **Apply the chain rule**:
   $\dfrac{\partial s}{\partial b} = \dfrac{\partial s}{\partial h} \dfrac{\partial h}{\partial z} \dfrac{\partial z}{\partial b}$
3. **Write out the Jacobians** from the table above:
   $u^{\top} \operatorname{diag}(f'(z))\, I$
4. **Simplify**: $u^{\top} \odot f'(z)$, where $\odot$ is the **Hadamard product**,
   element-wise multiplication of two vectors giving a vector — not a dot product, which
   would collapse them to a scalar (≈34:07).

Then comes the idea the rest of the lecture turns on. Computing $\partial s / \partial W$
needs
$\frac{\partial s}{\partial h} \frac{\partial h}{\partial z} \frac{\partial z}{\partial W}$,
and the first two factors are *identical* to the ones just computed for $b$ (slides 38–39).
So name that shared prefix
...
For the bias, $\partial z / \partial b$ is the identity, so $\partial s / \partial b$ is just
$\delta$. For the weights, $\partial s / \partial W = \delta^{\top} x^{\top}$ — the upstream
gradient times the local input signal, an outer product (slide 43).
...
derives it properly for a single weight: $W_{ij}$ is used only in computing $z_i$, and within
that it multiplies only $x_j$, so $\partial z_i / \partial W_{ij} = x_j$.
...
Strictly, $W \in \mathbb{R}^{n \times m}$ has $nm$ inputs and $s$ is one output, so
$\partial s / \partial W$ is a $1 \times nm$ Jacobian — a very long row vector. That is
correct and useless: the SGD update
...
$\partial s / \partial W$ is written $n \times m$.
...
second: the error signal $\delta$ arriving at a hidden layer has the same dimensionality as
that hidden layer.
...
**backpropagation** (slide 52), and it starts from $\partial s / \partial s = 1$.
...
Slides 59–69 work this end to end on $f(x, y, z) = (x + y) \cdot \max(y, z)$ at
$x = 1$, $y = 2$, $z = 0$. The forward pass gives $a = 3$, $b = 2$, $f = 6$. The backward
pass gives $\partial f/\partial x = 2$, $\partial f/\partial y = 5$,
$\partial f/\partial z = 0$. Manning then checks each answer numerically by
nudging an input, which is worth following because it makes the abstraction concrete:
moving $x$ to 1.1 gives $3.1 \times 2 = 6.2$, a rise of 0.2 for a change of 0.1, so the
gradient is 2 (≈54:22); moving $y$ to 2.1 gives $3.1 \times 2.1 = 6.51$, a rise of about
0.5, so the gradient is 5 (≈55:12).
...
The wrong way to backpropagate is to compute $\partial s / \partial b$, then independently
$\partial s / \partial W$, and so on, because the paths share a long prefix and you would
walk it repeatedly (slides 75–76). The right way is to compute all gradients in one backward
sweep, which is exactly the $\delta$ trick from the hand-derivation expressed as an
algorithm (slide 77).
...
with the perturbation $h \approx 10^{-4}$. Use the **two-sided** estimate, not the one-sided
one taught in calculus class — it is much more accurate and stable (≈1:09:59). It is easy to
get right and far too slow to train with, since it costs a re-evaluation of $f$ per
parameter, but it is the
