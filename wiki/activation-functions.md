...
transforms is that they **compose into a single linear transform**: $W_1 W_2 x = W x$
...
The original McCulloch and Pitts unit of 1943 was a **threshold** (slide 6), where
$\mathbb{1}(\cdot)$ is 1 when its condition holds and 0 otherwise, and $\theta$ is the
threshold:

$$\mathbb{1}(Wx > \theta) = \mathbb{1}(Wx - \theta > 0)$$
...
Slide 7 lays them out side by side, all written as functions of a scalar pre-activation $z$.

| Function | Definition | Range | Notes |
| --- | --- | --- | --- |
| **logistic** ("sigmoid") | $\dfrac{1}{1 + \exp(-z)}$ | $(0, 1)$ | still used where you want a probability |
| **tanh** | $\dfrac{e^z - e^{-z}}{e^z + e^{-z}}$ | $(-1, 1)$ | a rescaled logistic — see below |
| **hard tanh** | $-1$ if $z < -1$; $z$ if $-1 \le z \le 1$; $1$ if $z > 1$ | $[-1, 1]$ | piecewise-linear, avoids exponentials |
| **ReLU** | $\max(z, 0)$ | $[0, \infty)$ | the default first thing to try |
| **Leaky ReLU** | small positive slope (e.g. $0.01z$) below zero | $\mathbb{R}$ | removes the dead zone |
| **Parametric ReLU** | $az$ below zero, $a$ learned | $\mathbb{R}$ | the negative slope is a parameter |
| **Swish** | $z \cdot \operatorname{logistic}(z)$ | $\mathbb{R}$ | common in Transformers |
| **GELU** | $z \cdot P(X \le z),\ X \sim N(0,1) \approx z \cdot \operatorname{logistic}(1.702 z)$ | $\mathbb{R}$ | common in Transformers (BERT, RoBERTa) |

### tanh is a rescaled logistic

Worth knowing because the exponential formulas make it look unrelated (slide 7):

$$\tanh(z) = 2 \cdot \operatorname{logistic}(2z) - 1$$
...
$h = \operatorname{ReLU}(Wx + b_1)$ ([lecture 4](04-dependency-parsing.md), slide 44).
...
update of a simple RNN, $h^{(t)} = \sigma(W_h h^{(t-1)} + W_e e^{(t)} + b_1)$, most commonly
uses **tanh**, and Manning gives the reason: it is "balanced on the positive-negative side"
...
- **Sigmoid on the three gates.** $f$, $i$ and $o$ are sigmoids precisely *because* the range
  is $(0, 1)$: a gate's value has to be interpretable as "how much of this to let through", so
...
- **tanh on the cell content.** The candidate content $\tilde{c}^{(t)}$ and the read
  $h^{(t)} = o^{(t)} \odot \tanh c^{(t)}$ use tanh, because
...
proof sketch assumes $\sigma$ is the identity so that
$\partial h^{(t)} / \partial h^{(t-1)} = \operatorname{diag}(\sigma'(\cdot))\, W_h$ reduces to
...
- [Backpropagation](backpropagation.md) — where $f'(z)$ enters, as the diagonal local
  gradient $\operatorname{diag}(f'(z))$.
- [LSTM](lstm.md) — where sigmoid and tanh are used side by side for different purposes.
- [Vanishing and exploding gradients](vanishing-and-exploding-gradients.md) — where $\sigma'$
  shows up in every Jacobian factor.
